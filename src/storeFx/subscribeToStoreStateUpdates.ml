  ...
  |. Zustand.subscribe (fun state prevState ->
            ...
            | Some duration ->
                MediaSession.(
                  positionState ~duration
                    ~position:
                      (Js.Math.min_float state.player.currentTime duration)
                  |> setPositionState));
         if
         ...
         if
           ...
           || state.player.replayGainOptions
              != prevState.player.replayGainOptions
         then (
           let Subsonic.ReplayGain.
           ...
           let newReplayGainValue =
             Js.Math.min_float
               (10.0
               ** ((gain +. state.player.replayGainOptions.preAmp) /. 20.0))
               (1.0 /. peak)
           in
         ...
         (match
          ...
          with
         | None, _ | _, None -> None
         | Some currentSongId, Some prevSongId ->
             Some (currentSongId, prevSongId))
                ...
                if currentSongId != prevSongId then
                  ...
                  | Some credentials, Some duration ->
                      if
                        ...
                        (!timePlayed /. duration >= 0.5
                        || !timePlayed >= 4.0 *. 60.0)
                      then
                        Subsonic.Scrobble.make prevSongId ()
                        |. runAsync
                             ~deps:
                               Subsonic.Deps.{ credentials = Some credentials }
                        |> ignore;
                      timePlayed := 0.0));
