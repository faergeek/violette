...
let fetchWithUrlFx url =
  ...
  async (fun _ ->
      ...
      |> catch (fun _ -> err Error.NetworkError |> resolve))
                ...
                |> Json.Decode.oneOf
                     [
                       ...
                       (fun json ->
                         ...
                         Error.ApiError { code; message } |> err);
                     ]
              with issues -> Error.ValidationFailed { issues } |> err)
             |> resolve))
...
let makeRequest request of_json =
  ...
  | actual -> Error.NonJsonContentType { actual } |> err
