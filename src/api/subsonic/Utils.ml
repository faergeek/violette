...
let fetchWithUrlFx url =
  ...
  async (fun _ ->
      ...
      |> catch (fun _ -> err NetworkError |> resolve))
  ...
  async (fun _ ->
      ...
      |> then_ (fun json ->
             (try
                ...
                |> Json.Decode.oneOf
                     [
                       (fun json -> json |> of_json |> ok);
                       (fun json ->
                         let open Response in
                         let { response } = json |> failed_of_json in
                         let { code; message } = response.error in
                         ApiError { code; message } |> err);
                     ]
              with issues -> ValidationFailed { issues } |> err)
             |> resolve))
...
let makeRequest request of_json =
  ...
  | actual -> NonJsonContentType { actual } |> err
