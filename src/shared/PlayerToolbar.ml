...
let[@react.component] make ~queueId ~queueTriggerRef =
  let runStoreFx = StoreFx.useRun () in
  let runAsyncStoreFx = StoreFx.useRunAsync () in
  React.useEffect1
    (fun () ->
      StoreFx.subscribeToAudioEvents () |> runStoreFx |> Monads.Result.assertOk;
      None)
    [| runStoreFx |];
  React.useEffect2
    (fun () ->
           ...
           (fun event ->
             ...
             if activeElement == body then
               ...
               | "KeyK" | "Space" ->
                   ...
                   StoreFx.togglePaused () |> runAsyncStoreFx
                   |> Js.Promise.then_ (fun result ->
                          result |> Monads.Result.assertOk |> Js.Promise.resolve)
                   |> ignore
               | "KeyJ" | "ArrowLeft" ->
                   StoreFx.setCurrentTime (Fun.flip ( -. ) 10.0)
                   |> runAsyncStoreFx
                   |> Js.Promise.then_ (fun result ->
                          result |> Monads.Result.assertOk |> Js.Promise.resolve)
                   |> ignore
               ...
               | "ArrowUp" ->
                   event |. Dom.KeyboardEvent.preventDefault;
                   StoreFx.setVolume (Fun.flip ( +. ) 0.05)
                   |> runStoreFx |. Monads.Result.assertOk |> ignore
               | "ArrowDown" ->
                   event |. Dom.KeyboardEvent.preventDefault;
                   StoreFx.setVolume (Fun.flip ( -. ) 0.05)
                   |> runStoreFx |. Monads.Result.assertOk |> ignore
           ...
           ~f:
             (MediaSession.setActionHandler
                ~handler:
                  (Some
                     (fun event ->
                       event |> StoreFx.handleMediaSessionAction
                       |> runAsyncStoreFx
                       |> Js.Promise.then_ (fun result ->
                              result |> Monads.Result.assertOk
                              |> Js.Promise.resolve)
                       |> ignore)));
      Some
        (fun () ->
         ...
         (Store.Context.Consumer.make
            ...
            ~children:(fun queuedSongCount ->
              (Button.make ~ariaLabel:"Previous song" ~className:"grow p-3"
                 ...
                 ~onClick:(fun _ ->
                   StoreFx.goToPrevSong () |> runAsyncStoreFx
                   |> Js.Promise.then_ (fun result ->
                          result |> Monads.Result.assertOk |> Js.Promise.resolve)
                   |> ignore)
         ...
         (Store.Context.Consumer.make
            ...
            ~children:(fun queuedSongCount ->
              (Button.make ~ariaLabel:"Next song" ~className:"grow p-3"
                 ~disabled:(queuedSongCount == 0) ~variant:"icon"
                 ~onClick:(fun _ ->
                   StoreFx.goToNextSong () |> runAsyncStoreFx
                   |> Js.Promise.then_ (fun result ->
                          result |> Monads.Result.assertOk |> Js.Promise.resolve)
                   |> ignore)
