...
let[@mel.as "DropdownMenuContext"] context = Context.context
...
let[@mel.as "DropdownMenu"] root = Root.make
...
let[@mel.as "DropdownMenuTrigger"] trigger = Trigger.make
           ...
           (div ~ariaLabelledby:triggerId ~children
              ...
              ~onKeyDown:(fun event ->
                ...
                |. Option.bind (fun target ->
                       let role = DomExtra.role target in
...
let[@mel.as "DropdownMenuContent"] content = Content.make
...
let[@mel.as "DropdownMenuItem"] item = Item.make
